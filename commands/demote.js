...
exports.run = async (client, message, args) => {
    ...
    if(!username){
        ...
        footer: {
          text: "Created by Danial#0001 | Honey Brunch"
        }
      }
   });
    }
    ...
    message.channel.send({embed: {
        color: 9240450,
        description: `**Success!** ${username} has been demoted from ${rankNameInGroup} (${rankInGroup}) to ${demoteResponse.newRole.name} (${demoteResponse.newRole.rank})`,
        author: {
          name: message.author.tag,
          icon_url: message.author.displayAvatarURL()
        },
